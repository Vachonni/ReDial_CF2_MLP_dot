...
#                    if m == -1: break       # Reached the filling
                ...
                new_movies_indices = np.random.choice(range(48272), 100, replace=False)
...
##%%
#
## Relation Table for all movies (ReDial + Movie Lens) 
# 
#item_RT = [[None]] * 48272  
#
#for m in range(48272):       
#    _, title, genres = KB_of_movie(m)
#    item_RT[m] = [m, title, genres]
#  #  if m > 10: break
##
# 
#%%
...
## Creating a DataFrame and saving it
#
#df = pd.DataFrame(item_RT)
#df.columns = ['movie_UiD', 'title', 'genres']
#df.to_csv('movie_genres_RT.csv', index=False)
#
#
##%%
#
...
#df.columns = ['user_chrono_id', 'text']
#df.to_csv('user_chrono_RT.csv', index=False)
