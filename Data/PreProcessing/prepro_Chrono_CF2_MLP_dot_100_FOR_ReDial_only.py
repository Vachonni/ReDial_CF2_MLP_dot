...
# Path to dict of conversion from ReDialId to ReDialOrdinalId
ReDiD_2_ReDOrId_PATH = '/Users/nicholas/ReDial_Utils/ReDiD_2_ReDOrId.npy'
...
# Loading dict from ReDialId to ReDOrID
ReDiD_2_REDOrId = np.load(ReDiD_2_ReDOrId_PATH, allow_pickle=True).item()
...
for line in open(Conv_PATH, 'r'):
    ...
    for movieReDID, values in questions_dict.items():
        ...
        if values['liked'] == 0 or values['liked'] == 1:
            # Get the movie ReDOrID
            movieReDOrID = ReDiD_2_REDOrId[int(movieReDID)]
            # Get the rating according to the liked value
            rating = values['liked']
            l_ratings.append((movieReDOrID, rating))
    ...
    for message in conv_dict['messages']:
        ...
        if all_movies != []:
            ...
            # Get the movies mentionned as ReDOrId
            movies_found = [ ReDiD_2_REDOrId[int(film[1:])] for film in all_movies ]                
...
df.columns = ['data_idx', 'ConvID', 'qt_movies_mentioned', 'user_chrono_id', 'movie_ReDOrId', 'rating']
df.to_csv('/Users/nicholas/ReDial_CF2_MLP_dot/Data/DataReDial/Val100.csv', index=False)

# Creating a DataFrame and saving it

df = pd.DataFrame(train_data)
df.columns = ['data_idx', 'ConvID', 'qt_movies_mentioned', 'user_chrono_id', 'movie_ReDOrId', 'rating']
df.to_csv('/Users/nicholas/ReDial_CF2_MLP_dot/Data/DataReDial/Train100.csv', index=False)
